...
import inspect
import pickle
def loc(depth=0,p=True,a=[]):
    return None
    frame = inspect.currentframe().f_back
    output=(os.path.basename(frame.f_code.co_filename), frame.f_code.co_name, frame.f_lineno)
    if p:
        print(output)
    try:
        for x in a:
...
model = transformers.BertModel.from_pretrained(
    "./root/src/model/awesome_model_with_co", config=config
)
...
path_morphological = os.path.join(base_path, "./root/src/morphological/")
path_normalizer = os.path.join(base_path, "./root/src/normalizer/")
path_exception = os.path.join(base_path, "./root/src/alignment/de_en/exceptions.csv")
...
def awesome_alignment_preprocessing_wrapper(sent_srcs, sent_tgts):
    ...
    return ids_srcs, ids_trgs, sub2word_map_srcs, sub2word_map_trgs

def rechnen(padded_ids_tensor,model,device):
    model.eval()
    with torch.no_grad():
        padded_out_tensor = model(
            padded_ids_tensor.to(device), output_hidden_states=True
        )[2][align_layer].to("cpu")
    return padded_out_tensor

def ids_out(ids_srcs, ids_trgs, padded_out_src_tensor, padded_out_trg_tensor,):
    softmax_inter_list = []
    ...
    return softmax_inter_list

def awesome_alignment_postprocessing_wrapper(softmax_inter_list,
            sub2word_map_srcs,
            sub2word_map_trgs,
            pos_srcs,
            pos_trgs,
...
def awesome_alignment_batch(
    sentence_srcs, sentence_trgs, src_morphological_batch, trg_morphological_batch,idx,phase_num=None,
):
    sent_pos_save_path=f"./data/output/sent_pos/{idx}.pickle"
    ...
    if phase_num==0:
        ...
        pickle_save(path=padded_ids_tensor_save_path,data=(padded_ids_src_tensor ,padded_ids_trg_tensor ))
        pickle_save(path=moromoro_save_path,data=(ids_srcs, ids_trgs, sub2word_map_srcs,sub2word_map_trgs,pos_srcs,pos_trgs,sent_srcs,sent_tgts,))
        return []
    elif phase_num==2:
        padded_ids_src_tensor,padded_ids_trg_tensor=pickle_load(path=padded_ids_tensor_save_path)
        print("loaded")
        ...
        # post-processing
        alignmented_ls=awesome_alignment_postprocessing_wrapper(softmax_inter_list,sub2word_map_srcs,sub2word_map_trgs,pos_srcs,pos_trgs,sent_srcs,sent_tgts,)
        print("alignmented_ls",alignmented_ls)
...
def alignment_postprocess(alignmented, wordlist, test=False):
    ...
    for word_pair in alignmented:
        #[''], ['a'], ['n'], ['eine']
        src_pos_l = word_pair[2]#''
        src_word_l = word_pair[3]#a
        trg_pos_l = word_pair[2]# n
        trg_word_l = word_pair[1]# eine
        independent_index_src = -1
        for src_index, pos_tag in enumerate(src_pos_l):
            if pos_tag != "":
                independent_index_src = src_index #-1
        independent_index_trg = -1
        for trg_index, pos_tag in enumerate(trg_pos_l):
            if pos_tag != "":
                independent_index_trg = trg_index#0
...
def alignment_batch(corpus_rows, wordlist,phase_num,idx, test=False,):
        # pre-processing
    ...
    # post-processing
    #assert len(alignmented_ls) == len(corpus_rows) and len(corpus_rows)!=0
