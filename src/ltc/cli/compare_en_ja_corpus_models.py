"""Compare en_ja tracked-corpus outputs across two model specs."""
...
def parse_args(argv):
    parser = argparse.ArgumentParser(
        description="Compare en_ja tracked-corpus outputs across two model specs."
    )
    parser.add_argument(
        "--input",
    ...
    parser.add_argument(
        "--right-model",
        default="models/en-ja/awesome-align/production",
...
def build_subprocess_env(model_spec):
    env = os.environ.copy()
    env["ROOT"] = str(ROOT)
...
def run_audit(command, model_spec):
    result = subprocess.run(
        command,
        cwd=ROOT,
        env=build_subprocess_env(model_spec),
        capture_output=True,
        text=True,
        check=False,
    )
...
def format_text(payload, only_different=False):
    ...
    lines = [
        ...
        f"model: {payload['left']['model']}",
        "",
        "[right]",
        payload["right"]["runtime"]["note"],
        f"label: {payload['right']['label']}",
        f"model: {payload['right']['model']}",
        "",
        "[summary]",
        (
...
def main(argv=None):
    ...
    left_payload = run_audit(command, args.left_model)
    right_payload = run_audit(command, args.right_model)
